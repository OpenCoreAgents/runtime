...
Use a single **`REDIS_URL`** / `redis://` connection for shared memory, persisted runs (`wait` / `resume` across workers), and **`system_send_message`** over Redis Streams — the same key and stream layout as `@agent-runtime/adapters-upstash`, so you can swap between TCP and Upstash REST without changing engine code.
