...
| **POST** | `/agents/:agentId/run` | Body: `{ "message": string, "sessionId"?: string, "tenantId"?: string, "expiresAtMs"?: number, "extendSessionTtlMs"?: number, "projectId"?: string, "wait"?: boolean }`. **`tenantId`** is an optional sub-scope inside the effective **`projectId`**. **`wait`**: only with **`dispatch`** (+ **`queueEvents`**). Success JSON includes **`projectId`** and, when set, **`tenantId`**. |
| **POST** | `/agents/:agentId/resume` | Body: `{ runId, sessionId, resumeInput, "tenantId"?: string, "expiresAtMs"?: number, "extendSessionTtlMs"?: number, "projectId"?, "wait"? }`. Tenant-scoped runs require the same **`tenantId`** on resume. **Inline** mode needs **`runStore`** on **`AgentRuntime`**. **Queue** mode does not need **`runStore`** on the API process (worker must persist runs). |
| **POST** | `/agents/:agentId/continue` | Body: `{ runId, sessionId, message, "tenantId"?: string, "expiresAtMs"?: number, "extendSessionTtlMs"?: number, "projectId"?, "wait"? }`. Appends a user turn to a **`completed`** run (**same `runId`**). Tenant rules and **`runStore`** / **`dispatch`** behavior match **`resume`**. |
| **GET** | `/runs/:runId?sessionId=` | Requires **`runStore`**. **`sessionId`** must match the run when stored. **`run.projectId`** must exactly match the effective **`projectId`**. If the run has **`tenantId`**, callers must also send matching **`?tenantId=`** or the handler returns **403**. In **multi-project** mode, send **`X-Project-Id`** or **`?projectId=`** (same resolution as below). |
| **GET** | `/runs/:runId/history?sessionId=` | Same auth as **`GET /runs/:runId`** — returns **`Run.history`** (**`ProtocolMessage[]`**) plus **`runId`**, **`agentId`**, **`status`**, optional **`sessionId`** / **`projectId`** / **`tenantId`**. |
| **GET** | `/agents/:agentId/runs` | Requires **`runStore`**. Optional **`?status=`** (**`running` \| `waiting` \| `completed` \| `failed`**), **`?sessionId=`**, **`?tenantId=`**, **`?limit=`** (default **50**, max **100**). When **`sessionId`** is set, the router uses **`RunStore.listByAgentAndSession(projectId, agentId, sessionId, { tenantId?, status?, limit?, order: "desc" })`**; otherwise it falls back to **`listByAgent`** and filters to the effective **`projectId`** plus optional tenant. Without **`?tenantId=`**, tenant-scoped rows are hidden. |
...
### Optional tenant sub-scope

**`projectId`** remains the outer isolation boundary. **`tenantId`** is optional and only narrows runs inside that project. When a run is created with **`tenantId`**, later **`resume`**, **`continue`**, **`GET /runs/:runId`**, **`GET /runs/:runId/history`**, **`GET /agents/:agentId/runs`**, and **`GET /sessions/:sessionId/status`** must provide the same **`tenantId`** to see or mutate it. Omitting **`tenantId`** only matches project-scoped runs without a tenant.

Set **`requiredTenant: true`** to require a non-empty **`tenantId`** on run lifecycle and run-read routes. The router resolves tenant ids from **`X-Tenant-Id`**, then **`?tenantId=`**, then JSON **`body.tenantId`** on POSTs. This policy defaults to **`false`** for backward compatibility and does not require **`tenantId`** for non-run routes such as **`GET /agents`**.
...
| **R3 — Multi-tenant** | **`projectId`** / **`resolveProjectId`**, optional **`tenantId`** / **`requiredTenant`**, **`allowedProjectIds`**, **`resolveApiKey`**. |
