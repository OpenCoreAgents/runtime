...
> **Purpose** — Single source of truth to generate the entire `@opencoreagents` codebase from the `docs/core/` specifications. This document consolidates every interface, type, enum, module boundary, file path, dependency, and implementation contract into one actionable reference. It is NOT about CLI project scaffolding (see `docs/core/18-scaffold.md` for that).
...
| `core` | Security | **`projectId`** isolation in keys/adapters; prompt tool set = agent allowlist ∩ registry (+ optional **`AgentRuntime.allowedToolIds`**) — **`SecurityContext` is not applied in `ContextBuilder` yet** ([`08-scope-and-security.md`](../core/08-scope-and-security.md) §2, [`technical-debt-security-production.md`](./technical-debt-security-production.md#1-security-integrity-and-production-readiness) §1); principal auth stays in the host **`SecurityLayer`** | Unit for allowlist/prefixes; host integration for HTTP |
