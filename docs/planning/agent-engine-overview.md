...
| **BullMQ (primary job queue)** | **`@opencoreagents/adapters-bullmq`** — queue/worker helpers; **`dispatchEngineJob(runtime, payload)`** from **`@opencoreagents/core`** (re-exported) or `buildEngineDeps` + `executeRun` — [`core/06-adapters-infrastructure.md`](../core/06-adapters-infrastructure.md#job-queue-adapter-primary-bullmq). |
...
| [`core/05-adapters-contracts.md`](../core/05-adapters-contracts.md) | MemoryAdapter, ToolAdapter, RunStore contracts |
| [`core/06-adapters-infrastructure.md`](../core/06-adapters-infrastructure.md) | BullMQ, QStash, TCP Redis, Upstash packages |
