...
Planning doc: **minimum engine scope**, adapter choices for an MVP rollout, risks, and suggested build order. **Normative contracts** stay in [`docs/core/`](../core/README.md) — especially [05-adapters-contracts.md](../core/05-adapters-contracts.md) and [06-adapters-infrastructure.md](../core/06-adapters-infrastructure.md).

Related: [../core/02-architecture.md](../core/02-architecture.md) (components), [../core/07-definition-syntax.md](../core/07-definition-syntax.md) (`.define` + bootstrap), [../core/19-cluster-deployment.md](../core/19-cluster-deployment.md) (**`AgentRuntime`**, **RunStore**, adapters).
...
- States: **running**, **waiting**, **completed**, **failed** + **resume** after `wait` (in-process or via **`RunStore`** when another worker handles the next request — [19-cluster-deployment.md §3](../core/19-cluster-deployment.md)).
- Basic **hooks** at the run boundary (thought / action / observation / wait).

## Persistent adapters in MVP

Production deployments use **adapters** that satisfy the same core interfaces ([05-adapters-contracts.md](../core/05-adapters-contracts.md)); the engine loop does not change.
...
| **`@opencoreagents/adapters-bullmq` (priority)** | Typed **`createEngineQueue`** / **`createEngineWorker`**; **`dispatchEngineJob`** implemented in **`@opencoreagents/core`** and re-exported here — async `run` / `resume`, delayed jobs after `wait` (`reason: scheduled`); same engine API as SDK/REST — [06-adapters-infrastructure.md § Job queue](../core/06-adapters-infrastructure.md#job-queue-adapter-primary-bullmq). |
...
- Visual builder.
- **Product**-level “edit all definitions in a UI with no redeploy” (orchestration and auth around your DB) — out of **engine** MVP; the core already supports **JSON skill rows** + **`Skill.define` / `defineBatch`** with code **`execute`** ([07-definition-syntax.md §9.2b](../core/07-definition-syntax.md)).
...
Detailed contracts: [10-llm-adapter.md](../core/10-llm-adapter.md), [11-context-builder.md](../core/11-context-builder.md), [12-skills.md](../core/12-skills.md), [13-errors-parsing-and-recovery.md](../core/13-errors-parsing-and-recovery.md).
