...
| **`@opencoreagents/rest-api`** | **`createPlanRestRouter`**: **inline** **`Agent.run` / `resume`** (**`runtime`**) and/or **BullMQ** **`dispatch: { engine, … }`** (**202** + **`GET /jobs/:id`**, optional **`wait=1`** — same enqueue contract as **`dynamic-runtime-rest`**; **worker** must run **`createEngineWorker`** + **`dispatchEngineJob` / `runtime.dispatch`** with matching **`RunStore`** / **`dynamicDefinitionsStore`**). Still **no** product rate limits or idempotency in-router. **JSON body** limit **512kb**. **`GET /runs`** + shared **`RunStore`**: **`Run`** has **no `projectId`** — see **§7**. **`resumeHint.path`** is mount-relative. **Multi-project**: client-supplied tenant unless **`resolveProjectId`**; **`["*"]`** only behind strong auth. README: [`packages/rest-api/README.md`](../packages/rest-api/README.md). |
...
| **REST / MCP product APIs** | [14-consumers.md](./core/14-consumers.md) describes patterns. **Reference samples:** [`@opencoreagents/rest-api`](../packages/rest-api/) + [`examples/plan-rest-express/`](../examples/plan-rest-express/) — plan-shaped **sync** routes; [`examples/dynamic-runtime-rest/`](../examples/dynamic-runtime-rest/) — Redis definitions + BullMQ + per-job hydrate. **Shared gaps across samples:** **no rate limits**, not a substitute for product authZ (see **§7**). **`rest-api`**-specific: optional **`apiKey`**; multi-tenant **`projectId`** **spoofable** without **`resolveProjectId`**; **`GET /runs`** + shared **`RunStore`** — **§1** / **§7**. **`dispatch`** mode still needs a **worker** with the same queue + **`RunStore`** semantics as **`dynamic-runtime-rest`**. **`dynamic-runtime-rest`:** **no JSON-schema validation** on PUT bodies (casts only), fixed **`PROJECT_ID`** unless extended. **`_secrets`** on HTTP-tool PUT applies **only** to the API process; workers use **`HTTP_TOOL_SECRETS_JSON`** — invalid JSON there yields **empty** secrets (silent). Canonical flow: [`core/21-dynamic-runtime-rest.md`](./core/21-dynamic-runtime-rest.md). Roadmaps: [`plan-rest.md`](./plan-rest.md), [`plan-mcp.md`](./plan-mcp.md). |
...
| **`@opencoreagents/rest-api` + `GET /runs`** | Handler loads by **`runId`** only; **`sessionId`** query must match **`run.sessionId`** when set. There is **no** **`projectId`** on **`Run`** today — a caller who learns **`runId`** + **`sessionId`** can read status from **`GET /runs`** even if their **`X-Project-Id`** disagrees with the run’s tenant. Mitigate with **per-tenant `RunStore`**, **opaque unguessable ids**, edge auth that binds **`runId`** to the caller, or a future **`Run.projectId`** check in the HTTP layer. |
...
| Layer | Verify |
|-------|--------|
| **Identity** | No public route calls **`Agent.load(agentId, runtime, { session })`** without authenticating the caller and resolving **organization → `projectId`** (and **`endUserId`** for B2B2C). **`AgentRuntime`** construction must also sit behind the same trust boundary. If you use **`createPlanRestRouter`** in **multi-project** mode, **never** rely on client **`X-Project-Id`** alone — derive **`projectId`** with **`resolveProjectId`** from verified claims or map **`apiKey`** → tenant in your own middleware **before** the plugin. |
...
Last updated **2026-04-11** — **`@opencoreagents/rest-api`**: **`dispatch`** (BullMQ enqueue + **`GET /jobs`**) documented in **§1**, **§5**, snapshot; prior **`createPlanRestRouter`** rows (**§7**, **§9**) still apply. **Dynamic definitions:** **`DynamicDefinitionsStore`** facade (**`store.methods`** + **`store.Agent`**), **`@opencoreagents/dynamic-definitions`**, **`RedisDynamicDefinitionsStore`**, **`adapters-http-tool`**, **`hydrateAgentDefinitionsFromStore`** (strict missing-skill check), documented gaps in **§1**, **§5**, **§7**, **§8**, **§9**. **`dispatchEngineJob`** canonical path: **`packages/core/src/engine/dispatchJob.ts`** (BullMQ adapter re-exports). **§5** includes **`examples/dynamic-runtime-rest`** control-plane caveats. Prior: **`@opencoreagents/adapters-openai`** (**§3**); **§10** open-source checklist. Repository roadmap: [`plan.md` — Progress snapshot](./plan.md).
