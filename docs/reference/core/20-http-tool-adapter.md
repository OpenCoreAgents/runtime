...
Related: [Adapters contracts](./05-adapters-contracts.md) (`ToolAdapter`, `ToolRunner`), [Definition syntax](./06-definition-syntax.md) (`Tool.define`), [Scope and security](./11-scope-and-security.md) (SSRF, outbound calls), [Dynamic runtime REST](./21-dynamic-runtime-rest.md) (REST + store + sync).
...
- **Clustering:** Definitions and handlers are **per process**, like other `Tool.define` / `registerToolHandler` usage ([Cluster deployment](./16-cluster-deployment.md)): every worker must run the same bootstrap (or load the same catalog) before executing runs.
