...
Related: [Architecture](./02-architecture.md) (SecurityLayer before Engine), [Definition syntax](./06-definition-syntax.md) (definitions + registry), [Consumers](./15-consumers.md) (where auth attaches), [Cluster deployment](./16-cluster-deployment.md) (per-worker bootstrap). Full multi-tenancy (organizations, teams, end-users): [Multi-tenancy](./12-multi-tenancy.md).
...
When no `endUserId` is present (internal / operator use), `longTerm` falls back to `sessionId`-scoped keys. See [Multi-tenancy §4.3](./12-multi-tenancy.md) for the end-user memory model.
...
Definitions may be registered from **code** (`Tool.define` / `Skill.define` / `Agent.define`) or **hydrated** from a store into the same registry (`Skill.define` with JSON metadata + optional `execute`, or `Skill.defineBatch`); resolution rules are unchanged. See [Definition syntax](./06-definition-syntax.md) §9.2b.

**Today in core:** the prompt tool catalog is the agent allowlist (**explicit `tools` + tools from `skills`**) intersected with the **tool registry** and optional **`AgentRuntime.allowedToolIds`** — see `ContextBuilder` / `effectiveToolAllowlist`. **`SecurityContext` is not applied there yet** (no role-based hiding of tools in the LLM payload). Hosts must treat **definition resolution + agent allowlist** as the enforced surface unless they add their own filtering. Gap: [`technical-debt-security-production.md` §1](../../roadmap/technical-debt-security-production.md#1-security-integrity-and-production-readiness) (`SecurityContext` / `ContextBuilder`).
...
- `endUserId`: on **`Session`** drives **`MemoryAdapter`** keying for **`longTerm`** / **`vectorMemory`** when present; it is copied onto **`SecurityContext`** for **`ToolContext`** (see [Multi-tenancy §4.3](./12-multi-tenancy.md)).
...
**Inside core today:** **`ToolRunner`** enforces the **agent + skills (+ runtime) tool allowlist** and per-tool **`validate`**, but **does not** re-check **`SecurityContext.roles` / `scopes`** on each invocation. **`ContextBuilder`** receives **`securityContext`** on its input type but **does not use it** when building the tool list. See [`technical-debt-security-production.md` §1](../../roadmap/technical-debt-security-production.md#1-security-integrity-and-production-readiness).
...
| `Agent.load(agentId, runtime, { session })` | Principal authorized on the project; agent exists in that namespace; **`AgentRuntime`** wiring matches your deployment ([Cluster deployment](./16-cluster-deployment.md)). |
...
Bus contract, `system_send_message` tool, and coordination with `wait`/`resume`: [Multi-agent communication](./14-communication-multiagent.md).
...
Detail: [Multi-tenancy §3](./12-multi-tenancy.md).
...
See also [mvp.md](../../roadmap/mvp.md) (shared Redis risks) and [Definition syntax](./06-definition-syntax.md) (`security`, `projectId`, `scope`).
...
| **Runs** | Treat **`runId`** as sensitive; bind **resume** to the same **session/tenant** as create (see [`technical-debt-security-production.md` §1](../../roadmap/technical-debt-security-production.md#1-security-integrity-and-production-readiness)). Use **`RunStore`** + queue **idempotency** in clusters ([`technical-debt-security-production.md` §2](../../roadmap/technical-debt-security-production.md#2-multi-worker-concurrency-and-integrity)). |
...
| **Infra** | Redis/vector **TLS**, **ACL**, key **prefixes** per environment; identical **bootstrap** (**`AgentRuntime`** wiring + definitions) on every worker ([Cluster deployment](./16-cluster-deployment.md)). |

Full gap list: [`technical-debt-security-production.md` §1–§3](../../roadmap/technical-debt-security-production.md) (sections **1–3** in that file; hub: [`technical-debt.md`](../../roadmap/technical-debt.md)).
...
- **Organizations, teams, and end-users** are resolved to `SecurityContext` fields in the platform layer — the engine loop is unaware of these concepts. See [Multi-tenancy](./12-multi-tenancy.md).
