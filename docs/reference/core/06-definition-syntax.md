...
Related: [Cluster deployment §2](./16-cluster-deployment.md) (bootstrap — **`AgentRuntime`**, built-in tools, `runStore`), [Execution model](./03-execution-model.md) (wait / resume).
...
| `llm` | yes* | *Required if the engine calls the adapter unless **`AgentRuntime`** supplies a matching adapter via **`llmAdapter`** / **`llmAdaptersByProvider`** — see [Cluster deployment §2](./16-cluster-deployment.md). |
...
Validation, errors, and re-prompt when JSON is invalid: [Errors, parsing, and recovery](./10-errors-parsing-and-recovery.md).
...
// Built-ins: memory + vector + messaging from AgentRuntime; RAG/file tools from @opencoreagents/rag — all `system_*` ids; shapes in §8 / 18-rag-pipeline.md; not re-defined here unless you replace defaults.
...
**Cluster**: each worker registers locally after reading the same data (boot or pub/sub). No Redis client in `@opencoreagents/core` — see [Cluster deployment](./16-cluster-deployment.md) §1.1.
...
**`onWait`**: if the callback returns a **string**, the engine **continues the same run in-process** (equivalent to `resume` with `type: "text"`). If it returns **`undefined`**, the run stays **`waiting`** — use **`agent.resume(runId, input)`** (requires **`runStore`** on **`AgentRuntime`** for cross-worker resume; see [Cluster deployment §3](./16-cluster-deployment.md)).
...
When `endUserId` is present, the MemoryAdapter scopes `longTerm` and `vectorMemory` by `endUserId` instead of `sessionId`, enabling persistence across conversations. See [Multi-tenancy §4](./12-multi-tenancy.md).
...
**Custom orchestration (queue workers, HTTP handlers)** — same loop as `RunBuilder`, without the fluent API: **`buildEngineDeps(agent, session, runtime)`** → **`createRun`** → **`executeRun`** with **`startedAtMs`** (and **`resumeMessages`** after a `wait`). If you bypass `Agent.resume`, persist **`waiting`** runs via **`runStore`** yourself. See [Consumers](./15-consumers.md) and [Cluster deployment](./16-cluster-deployment.md).
...
Full scope (session, run, store keys) and **SecurityLayer**: [Scope and security](./11-scope-and-security.md).
...
Detail on `SecurityContext` fields and principal kinds: [Scope and security](./11-scope-and-security.md). Full multi-tenancy model: [Multi-tenancy](./12-multi-tenancy.md).
