...
Related: [Adapters contracts](./05-adapters-contracts.md) (adapter contracts), [RAG pipeline](./18-rag-pipeline.md) (RAG tools that consume these utils).
...
**Security note**: `resolveSource` itself does not enforce access control. The calling **tool** must verify that the source is allowed for the current `projectId` and `SecurityContext` before invoking this util. See [Scope and security](./11-scope-and-security.md).
...
|-----------|---------|------|
| `token-counter/` | Accurate token counting for context budget in Context Builder ([Context builder](./08-context-builder.md)). | MVP or MVP+ |
