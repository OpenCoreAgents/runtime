...
Related: [Adapters infrastructure](./13-adapters-infrastructure.md) (MessageBus backends, Redis packages), [Scope and security](./11-scope-and-security.md) (isolation), [Consumers](./15-consumers.md), [Cluster deployment](./16-cluster-deployment.md) (**`messageBus`** on **`AgentRuntime`** on every worker for cross-process delivery; **`runStore`** when **`wait`** spans workers). Optional brainstorm: [Multi Agent Rest Sessions](../../archive/brainstorm/07-multi-agent-rest-sessions.md).
...
- **Same `projectId` only** by default; validate in `system_send_message` and in the bus ([Scope and security](./11-scope-and-security.md)).
