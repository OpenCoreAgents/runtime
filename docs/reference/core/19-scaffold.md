...
Related: [Definition syntax](./06-definition-syntax.md) (what gets generated), [Consumers](./15-consumers.md) (CLI consumer), [Adapters contracts](./05-adapters-contracts.md) (adapter config templates). Monorepo package layout and implementation phases: [`../roadmap/scaffold.md`](../../roadmap/scaffold.md) (not duplicated here).
...
Adds `agents/` with two sample agents, `config/message-bus.ts` for inter-agent communication ([Multi-agent communication](./14-communication-multiagent.md)), and a shared `skills/` directory.
...
Each section maps to an adapter contract in [Adapters contracts](./05-adapters-contracts.md). Fields with `process.env` reference the generated `.env.example`.
...
File-level structure of each template module is specified in [`../roadmap/scaffold.md`](../../roadmap/scaffold.md) §1 (`packages/scaffold`).
...
| Doc | Connection |
|-----|------------|
| [Definition syntax](./06-definition-syntax.md) | Scaffold generates files that follow `Tool.define`, `Skill.define`, `Agent.define` syntax. Runtime-defined skills may use `SkillDefinitionPersisted` + `Skill.define(..., execute?)` (§9.2b there). |
| [Adapters contracts](./05-adapters-contracts.md) | `config/runtime.ts` maps to adapter contracts; `--adapter` flag selects the preset. |
| [Scope and security](./11-scope-and-security.md) | `config/security.ts` bootstraps `SecurityLayer` defaults. |
| [Consumers](./15-consumers.md) | The CLI `init` and `generate` commands are consumer-level operations. |
| [Multi-tenancy](./12-multi-tenancy.md) | Multi-tenant scaffolds include `projectId` in generated definitions. |
