...
Execution model: [Execution model](./03-execution-model.md). `Step` shape: [Definition syntax](./06-definition-syntax.md) §5. LLM: [LLM adapter](./07-llm-adapter.md).
...
|----------|----------|----------------|
| **LLM transport** | Timeout, network, 503 | Yes, bounded ([10](./07-llm-adapter.md)) |
...
- Caller passes `AbortSignal` through to **LLM Adapter** ([10](./07-llm-adapter.md)) and, when relevant, long-running tools (fetch).
...
| **Tool** | Optional **`toolTimeoutMs`** on **`AgentRuntime`** → `ToolRunner` races tool `execute`; on expiry, **`ToolTimeoutError`** (`TOOL_TIMEOUT`) becomes an error `observation` (`success: false`) like other tool failures. Does not cancel in-flight work. |
...
3. **Validate** minimal schema:
   ...
   - Required fields per type (`content` or `tool`+`input`, etc.) per [Definition syntax](./06-definition-syntax.md) §5.
