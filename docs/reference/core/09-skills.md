...
Related: [Architecture](./02-architecture.md); [Adapters contracts](./05-adapters-contracts.md) (ToolRunner / tool contracts); [Definition syntax](./06-definition-syntax.md) (`Skill.define` / `defineBatch`, §9.2b); [Scope and security](./11-scope-and-security.md) (definition resolution); [Context builder](./08-context-builder.md); [Cluster deployment](./16-cluster-deployment.md) §1.1 (registry per worker).

**Runnable:** load OpenClaw / AgentSkills-style **`SKILL.md`** from disk with **`@opencoreagents/skill-loader-openclaw`** — [examples/load-openclaw-skills](../../../examples/load-openclaw-skills/).
...
2. Resolve each id against store **project → global** ([Scope and security](./11-scope-and-security.md)).
3. Union tools referenced by all resolved skills + direct agent tools → candidate set.
4. Intersect with the **tool registry** and optional **`AgentRuntime.allowedToolIds`** → set exposed to **Context Builder** ([Context builder](./08-context-builder.md)); **`SecurityContext` does not further narrow tools in core today** ([Scope and security](./11-scope-and-security.md) §2).
