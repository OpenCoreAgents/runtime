...
Related: [Architecture](./02-architecture.md) (components and data flow), [Cluster deployment](./16-cluster-deployment.md) (per-process **`AgentRuntime`**, **`RunStore`**, **`messageBus`**).
...
| Layer | Role |
|-------|------|
| **LLM Adapter** | `generate(...)`: swappable provider and model. See [LLM adapter](./07-llm-adapter.md). |
| **Agent Engine** | Loop, context, decision parsing, tools, memory, wait/resume. Errors/parsing: [Errors, parsing, and recovery](./10-errors-parsing-and-recovery.md). |
| **Tools** | Adapter registry; the model **proposes**, the engine **executes**. |
| **Context Builder** | Assembles the prompt; see [Context builder](./08-context-builder.md). |
| **Skills** | Higher-level capabilities; see [Skills](./09-skills.md). |
