...
Related: [HTTP tool adapter](./20-http-tool-adapter.md) (JSON HTTP tools), [Definition syntax](./06-definition-syntax.md) (`Tool.define` / `Skill.define` / `Agent.define`), [Cluster deployment](./16-cluster-deployment.md) (per-process registry, worker bootstrap), [Consumers](./15-consumers.md) (REST as consumer), [Adapters infrastructure](./13-adapters-infrastructure.md) (BullMQ, Redis adapters), [`plan-rest.md`](../../roadmap/plan-rest.md).
...
| **BullMQ** | **`run` / `resume`** jobs; **one worker** processes a given job at a time; payload includes **`projectId`**, **`agentId`**, session/run ids, user input. See [Adapters infrastructure § Job queue](./13-adapters-infrastructure.md#job-queue-adapter-primary-bullmq). |
...
The engine registry is **per Node process** ([`registry.ts` comment](./16-cluster-deployment.md)). For **queue workers**:
...
Nothing in the dynamic store should mark itself global — that would break tenant isolation expectations. See also [Multi-tenancy](./12-multi-tenancy.md).
