...
Related: [Architecture](./02-architecture.md); **`llmAdapter` / `llmAdaptersByProvider`** on **`AgentRuntime`** — [Cluster deployment §2](./16-cluster-deployment.md); agent **`llm`** field vs defaults — [Definition syntax](./06-definition-syntax.md) §1; prompt assembly — [Context builder](./08-context-builder.md); failures and retries — [Errors, parsing, and recovery](./10-errors-parsing-and-recovery.md).
...
- Recommendation: JSON-style `responseFormat` or system instruction + engine **validation** ([Errors, parsing, and recovery](./10-errors-parsing-and-recovery.md)).
...
- `maxOutputTokens` on the request; if `finishReason === "length"`, the engine may mark **parse failure** or a controlled error `Step` ([13](./10-errors-parsing-and-recovery.md)).
