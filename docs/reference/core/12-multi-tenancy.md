...
Related: [Scope and security](./11-scope-and-security.md) (scope levels, SecurityLayer), [Definition syntax](./06-definition-syntax.md) (Session, SecurityContext types), [Adapters contracts](./05-adapters-contracts.md) (memory key patterns), [Dynamic runtime REST](./21-dynamic-runtime-rest.md) (REST/store definitions).

Definitions ingested through **`@opencoreagents/dynamic-definitions`** are **project-scoped only**; `scope: "global"` is **not allowed** there. **`scope: "global"`** remains for **built-ins** (`system_*`, etc.) and for **`Tool.define` / `Skill.define` in your application code** when you intentionally share a catalog across all projects. See [Dynamic runtime REST §3.1](./21-dynamic-runtime-rest.md#31-project-isolation-vs-scope-global).
