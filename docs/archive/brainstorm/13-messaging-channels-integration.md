...
**Brainstorm / design sketch.** How an external chat channel fits **`Session`**, **`Run`**, **`wait`/`resume`**, and what stays **outside** the engine. Complements [`07-multi-agent-rest-sessions.md`](./07-multi-agent-rest-sessions.md) and [`../core/15-consumers.md`](../../reference/core/15-consumers.md).
...
- **`ContextBuilder`**: builds LLM **`messages`** from **this run’s** `userInput` + `history` + optional `resumeMessages` — see [`../core/08-context-builder.md`](../../reference/core/08-context-builder.md).
...
Webhook handlers should usually **ack immediately** and **enqueue** work. Long **`await agent.run()`** inside the webhook may hit **HTTP timeouts** on the provider side. Use a **queue + worker** pattern; align with [`../core/16-cluster-deployment.md`](../../reference/core/16-cluster-deployment.md) for multi-worker **`RunStore`**.
...
- [`../core/04-protocol.md`](../../reference/core/04-protocol.md) — step types.
- [`../core/08-context-builder.md`](../../reference/core/08-context-builder.md) — how LLM messages are assembled.
- [`../core/16-cluster-deployment.md`](../../reference/core/16-cluster-deployment.md) — **`RunStore`**, workers, sessions.
