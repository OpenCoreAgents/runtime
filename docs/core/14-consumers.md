...
HTTP/JSON: list agents, `POST` run/resume, memory, logs, inter-agent send, define. Same semantics as the SDK; auth and rate limits before the engine. For **plan-rest-shaped** URLs (**`GET /agents`**, **`POST /agents/:id/run`**, …) use [**`@opencoreagents/rest-api`**](../../packages/rest-api/) (**`createPlanRestRouter`**) and [**`examples/plan-rest-express/`**](../../examples/plan-rest-express/). The plugin supports **inline** **`Agent.run`** (**`runtime`**) and/or **BullMQ** **`dispatch`** (same enqueue shape as **`dynamic-runtime-rest`**), a fixed **`projectId`** or **multi-project** resolution (**`X-Project-Id`**, **`?projectId=`**, **`body.projectId`**), optional **`allowedProjectIds`** (use **`["*"]`** with care), optional **`agentIds`** allowlist, and optional **`apiKey`** — details: [package README](../../packages/rest-api/README.md). For a richer BFF with **`/v1/chat`** and SSE, use [**`examples/real-world-with-express/`**](../../examples/real-world-with-express/). For **async** (BullMQ + `dispatch`), use [**`examples/dynamic-runtime-rest/`**](../../examples/dynamic-runtime-rest/). **`plan-rest.md`** § *Easiest HTTP server (today)* ties these together. Broader brainstorm: [07-multi-agente-rest-sesiones.md](../brainstorm/07-multi-agente-rest-sesiones.md).
