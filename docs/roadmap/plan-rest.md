...
| **What you build** | **`AgentRuntime`** + **`Agent.load` / `run` / `resume`** (or **`dispatch` / `dispatchEngineJob`** on workers). HTTP is optional glue — [`16-cluster-deployment.md`](../reference/core/16-cluster-deployment.md). |

**Non-goals:** Business auth, rate limits, multi-region — **outside** `packages/core`. Same as [`11-scope-and-security.md`](../reference/core/11-scope-and-security.md).
...
| **POST** | `/agents/:agentId/run` | Body: **`message`** (required), optional **`sessionId`**, **`tenantId`**, **`expiresAtMs`**, **`extendSessionTtlMs`**, **`projectId`** (multi-project), **`wait`** (with **`dispatch`**). Inline: **`Agent.run`**. JSON responses include effective **`projectId`** and optional **`tenantId`**. Queue: **`engine.addRun`** → **202** + **`jobId`** / **`projectId`** / **`statusUrl`**, or **`?wait=1`** / **`wait: true`** with **`queueEvents`**. |
| **POST** | `/agents/:agentId/resume` | Body: **`runId`**, **`sessionId`**, **`resumeInput` `{ type, content }`**, optional **`tenantId`**, **`expiresAtMs`**, **`extendSessionTtlMs`**, **`projectId`**, **`wait`**. Tenant-scoped runs require matching **`tenantId`**. Inline needs **`runStore`** on **`AgentRuntime`**. JSON responses include effective **`projectId`** when enqueuing or completing inline. |
| **GET** | `/runs/:runId?sessionId=` | Snapshot from **`runStore`**; **`sessionId`** query required; checks session match; **`run.projectId`** must match effective project (**403**). Tenant-scoped runs require matching **`?tenantId=`**. Response may include **`projectId`** and **`tenantId`**. |
| **GET** | `/runs/:runId/history?sessionId=` | Full **`Run.history`** (**`ProtocolMessage[]`**) — same **`sessionId`** / **`projectId`** / optional **`tenantId`** rules as **`GET /runs/:runId`**. |
| **GET** | `/agents/:agentId/runs` | With **`runStore`**: dashboard-style list — optional **`?status=`**, **`?sessionId=`**, **`?tenantId=`**, **`?limit=`** (default **50**, max **100**). When **`sessionId`** is set, the router can use **`RunStore.listByAgentAndSession(projectId, agentId, sessionId, { tenantId?, ... })`** for session-scoped retrieval instead of loading all runs for the agent. Rows must match the effective **`projectId`**; omitting **`tenantId`** hides tenant-scoped rows. |
| **GET** | `/jobs/:jobId` | Only with **`dispatch`** — BullMQ job state + **`run`** summary when completed. |

**Options (summary):** **`runtime`** (inline + memory + inter-agent send when **`runtime.config.messageBus`** is set), **`dispatch: { engine, queueEvents?, jobWaitTimeoutMs? }`** (BullMQ, peers **`@opencoreagents/adapters-bullmq`** + **`bullmq`**), **`projectId`** (fixed tenant) or multi-tenant via **`X-Project-Id`** → **`?projectId=`** → **`body.projectId`**, optional **`tenantId`** via **`X-Tenant-Id`** → **`?tenantId=`** → **`body.tenantId`**, **`requiredTenant`** for run routes, **`resolveProjectId(req)`**, **`allowedProjectIds`**, **`agentIds`**, **`apiKey`** / **`resolveApiKey(req, res)`** (project/tenant resolved **before** auth — use **`getRuntimeRestRouterProjectId(res)`** and **`getRuntimeRestRouterTenantId(res)`** for scoped secrets), **`runStore`**, **`swagger`** (OpenAPI **3.0** + Swagger UI on **`/openapi.json`** / **`/docs`** by default).
...
| **Dynamic definitions package** | [`@opencoreagents/dynamic-definitions`](../../packages/dynamic-definitions/) — [`21-dynamic-runtime-rest.md`](../reference/core/21-dynamic-runtime-rest.md). |
...
| **POST** | `/agents/:id/run` | User message → run (job or inline) | ✓ (`message` body; optional **`expiresAtMs`** / **`extendSessionTtlMs`** in HTTP) |
| **POST** | `/agents/:id/resume` | Resume **`wait`** | ✓ |
| **POST** | `/agents/:id/continue` | New user turn on **`completed`** run (**same `runId`**) | ✓ (`message` body) |
| **GET** | `/runs/:runId` | Run snapshot | ✓ ( **`sessionId`** query; exact **`run.projectId`** match; optional **`tenantId`** match for tenant-scoped runs) |
| **GET** | `/runs/:runId/history` | Full **`Run.history`** | ✓ (same **`sessionId`** / project / optional tenant rules) |
| **GET** | `/agents/:id/memory` | Scoped memory read | ✓ (**`MemoryAdapter.query`**; needs router **`runtime`**) |
| **GET** | `/agents/:id/runs` | List run summaries (dashboard) | ✓ (**`RunStore.listByAgentAndSession(projectId, agentId, sessionId, { tenantId?, ... })`** when **`sessionId`** is supplied, else **`listByAgent`** with project/tenant filtering) |
...
| **R2 — Async** | **202** + poll | **`dispatch`** + **`GET /jobs/:jobId`**, **`wait=1`** (tested); **`isBullmqJobWaitTimeoutError`** for **504** vs **502** on **`waitUntilFinished`**; worker = [`dispatchEngineJob`](../../packages/core/src/engine/dispatchJob.ts) / [`21-dynamic-runtime-rest.md`](../reference/core/21-dynamic-runtime-rest.md). |
| **R3 — Multi-tenant** | Safe tenancy | Fixed / resolved **`projectId`**, optional **`tenantId`** sub-scope for runs, **`requiredTenant`** policy, **`allowedProjectIds`**, **`resolveApiKey`**; **`Run.projectId`** + **`GET /runs`** **403** on mismatch. |
...
- [`archive/brainstorm/07-multi-agent-rest-sessions.md`](../archive/brainstorm/07-multi-agent-rest-sessions.md)
- [`core/15-consumers.md`](../reference/core/15-consumers.md)
...
- Cluster: [`core/16-cluster-deployment.md`](../reference/core/16-cluster-deployment.md)
- Dynamic runtime: [`core/21-dynamic-runtime-rest.md`](../reference/core/21-dynamic-runtime-rest.md)
