...
> Actionable step-by-step guide to build the `@opencoreagents` monorepo from zero. Each phase has a **gate** — criteria that must pass before moving on. Derived from `docs/roadmap/scaffold.md` and `docs/reference/core/`.

**Canonical contracts:** [`docs/reference/core/README.md`](../reference/core/README.md) (engine, adapters, cluster). **Monorepo package map / file tree:** [`docs/roadmap/scaffold.md`](./scaffold.md) §0–§1.
...
**Runnable examples:** [`examples/minimal-run`](../../examples/minimal-run/) (mock LLM, **`AgentRuntime`**, no keys); [`examples/load-openclaw-skills`](../../examples/load-openclaw-skills/) (**`@opencoreagents/skill-loader-openclaw`**, **`SKILL.md`** packs, **`registerOpenClawExecTool`**, **`defaultSkillIdsGlobal`**; mock LLM, no keys); [`examples/openai-tools-skill`](../../examples/openai-tools-skill/) (`OpenAILLMAdapter` + tool + skill, **`OPENAI_API_KEY`**); [`examples/console-wait`](../../examples/console-wait/) (**`onWait`** + stdin); [`examples/rag`](../../examples/rag/) (**`registerRagToolsAndSkills`**, **`registerRagCatalog(runtime, …)`**, **`system_ingest_rag_source`** / **`system_vector_search`**; optional **`start:openai`**); [`examples/rag-contact-support`](../../examples/rag-contact-support/) (same RAG stack + custom **`contact_support`** tool, scripted LLM).
...
| **RunStore + resume** (`AgentRuntime` + **`runStore`**, `InMemoryRunStore`, **`RedisRunStore`** / `UpstashRunStore`, `Agent.resume`, `RunBuilder` persistence) | **Done** — see `docs/reference/core/16-cluster-deployment.md` §3 |
...
For package-level detail, see **`docs/roadmap/scaffold.md` §0.8** and **§12**. Known gaps and deferrals: [**`docs/roadmap/technical-debt.md`**](./technical-debt.md) (hub; triage **[security & production](./technical-debt-security-production.md)** first).
...
| 1.8 | `SecurityContext` + `SessionOptions`; embedded context via **`securityContextForAgent`** (`buildEngineDeps`) | `src/security/types.ts`, `src/engine/buildEngineDeps.ts` | No in-core HTTP **`SecurityLayer`** module — hosts authenticate **before** **`Agent.load`**; see [`11-scope-and-security.md`](../reference/core/11-scope-and-security.md) |
...
**Status:** **`packages/adapters-bullmq`** provides **`createEngineQueue`** and **`createEngineWorker`**. **`dispatchEngineJob`** and typed **`EngineJobPayload`** (`run` / `resume`) are implemented in **`packages/core`** and **re-exported** from **`adapters-bullmq`**. Workers still call the same engine entry points as the SDK (`Agent.run` / `Agent.resume` via `dispatchEngineJob`, or `buildEngineDeps` + `executeRun` for lower-level control). See `docs/reference/core/16-cluster-deployment.md` §4.
