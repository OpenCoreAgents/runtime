...
Fifteen workspace packages (`core`, `utils`, `adapters-openai`, `adapters-anthropic`, `adapters-upstash`, `adapters-redis`, **`adapters-bullmq`**, **`adapters-http-tool`**, **`dynamic-definitions`**, **`conversation-gateway`**, **`rest-api`**, `rag`, `scaffold`, `cli`, **`code-skills`**) build and test together. **BullMQ** (`@opencoreagents/adapters-bullmq`) is the **priority** path for background runs and workers (`createEngineQueue`, `createEngineWorker`; **`dispatchEngineJob(runtime, payload)`** lives in **`@opencoreagents/core`** and is re-exported from **`adapters-bullmq`**). **TCP Redis** adapters are the **default** for shared engine state; pair with the same Redis for queues when it fits ops. **Upstash REST** + **Upstash Vector** when you want HTTP-only Redis. **Per-tool timeouts**: set **`toolTimeoutMs`** on **`AgentRuntime`**. **CI**: `pnpm turbo run build test lint` ([`.github/workflows/ci.yml`](../../.github/workflows/ci.yml)). Roadmap: [`plan.md`](./plan.md); gaps: [`technical-debt.md`](./technical-debt.md) (hub — production first: [`technical-debt-security-production.md`](./technical-debt-security-production.md)).
...
| **BullMQ (primary job queue)** | **`@opencoreagents/adapters-bullmq`** — queue/worker helpers; **`dispatchEngineJob(runtime, payload)`** from **`@opencoreagents/core`** (re-exported) or `buildEngineDeps` + `executeRun` — [`core/13-adapters-infrastructure.md`](../reference/core/13-adapters-infrastructure.md#job-queue-adapter-primary-bullmq). |
...
| **MCP** | **Model Context Protocol**: IDEs and assistants expose **tools** to their model; an MCP server can **proxy** those tool calls to your REST or SDK. MCP wires interoperability; it does **not** replace engine memory, history, or ToolRunner ([`core/01-purpose.md`](../reference/core/01-purpose.md), [`core/15-consumers.md`](../reference/core/15-consumers.md)). |
...
The architecture, contracts, protocol, and adapter interfaces are fully documented in `docs/reference/core/`. The engine code does not exist yet. That is what we are building.
...
| Doc | What it covers |
|-----|----------------|
| [`core/01-purpose.md`](../reference/core/01-purpose.md) | What the engine is and what it is NOT |
| [`core/03-execution-model.md`](../reference/core/03-execution-model.md) | Run entity, states, loop, wait/resume |
| [`core/04-protocol.md`](../reference/core/04-protocol.md) | Message protocol and invariants |
| [`core/05-adapters-contracts.md`](../reference/core/05-adapters-contracts.md) | MemoryAdapter, ToolAdapter, RunStore contracts |
| [`core/13-adapters-infrastructure.md`](../reference/core/13-adapters-infrastructure.md) | BullMQ, QStash, TCP Redis, Upstash packages |
| [`planning/mvp.md`](./mvp.md) | MVP scope and suggested implementation order |
...
| Doc | Topic |
|-----|-------|
| [`core/02-architecture.md`](../reference/core/02-architecture.md) | Internal components and responsibilities |
| [`core/06-definition-syntax.md`](../reference/core/06-definition-syntax.md) | Full `Tool.define` / `Skill.define` / `Agent.define` shapes and types |
| [`core/11-scope-and-security.md`](../reference/core/11-scope-and-security.md) | Multi-tenant scopes and SecurityLayer |
| [`core/14-communication-multiagent.md`](../reference/core/14-communication-multiagent.md) | MessageBus, `system_send_message`, request-reply patterns |
| [`core/07-llm-adapter.md`](../reference/core/07-llm-adapter.md) | LLMAdapter contract — multi-provider, streaming, errors |
| [`core/08-context-builder.md`](../reference/core/08-context-builder.md) | Prompt assembly, truncation, **effectiveToolAllowlist**; **SecurityContext** not used to filter prompt tools yet ([`11-scope-and-security.md`](../reference/core/11-scope-and-security.md) §2) |
| [`core/09-skills.md`](../reference/core/09-skills.md) | Skills vs tools — resolution, model visibility, imperative execute |
| [`core/10-errors-parsing-and-recovery.md`](../reference/core/10-errors-parsing-and-recovery.md) | Failures, timeouts, bounded re-prompt |
| [`core/15-consumers.md`](../reference/core/15-consumers.md) | SDK, CLI, REST, MCP, webhooks, cron |
| [`core/20-http-tool-adapter.md`](../reference/core/20-http-tool-adapter.md) | JSON-configured outbound HTTP tools (`adapters-http-tool`) |
| [`core/21-dynamic-runtime-rest.md`](../reference/core/21-dynamic-runtime-rest.md) | Dynamic agents/skills/HTTP tools via store + sync (`dynamic-definitions`, example `dynamic-runtime-rest`) |
...
Documents in [`archive/brainstorm/`](../archive/brainstorm/) are the design process notes — useful for understanding *why* decisions were made, not as final specification. The PRD overview is at [`archive/brainstorm/12-prd-overview.md`](../archive/brainstorm/12-prd-overview.md).
