...
> **Purpose** — Single source of truth to generate the entire `@opencoreagents` codebase from the `docs/reference/core/` specifications. This document consolidates every interface, type, enum, module boundary, file path, dependency, and implementation contract into one actionable reference. It is NOT about CLI project scaffolding (see `docs/reference/core/19-scaffold.md` for that).

### Relationship to `docs/reference/core/19-scaffold.md`

| Document | Audience | Contents |
|----------|----------|----------|
| **`docs/roadmap/scaffold.md` (this file)** | Implementers of the monorepo | Turborepo layout, **all** `packages/*` boundaries, dependency graph, canonical types, implementation phases, testing matrix. |
| **`docs/reference/core/19-scaffold.md`** | Users of the product CLI / programmatic API | `init` / `generate` commands, flags, generated **end-user** project layout, post-scaffold checklist. |
...
| File | Purpose |
|------|---------|
| `package.json` | Name scoped `@opencoreagents/*`, `"main": "dist/index.js"`, `"types": "dist/index.d.ts"`, `"exports"` field, internal `"dependencies"` via `"workspace:*"`. `tsup` and `typescript` are root `devDependencies` — do not duplicate in per-package `devDependencies`. |
...
```jsonc
{
  "name": "@opencoreagents/PACKAGE_NAME",
  "version": "0.0.0",
  "private": true,
  "type": "module",
  "main": "dist/index.js",
...
@opencoreagents/adapters-http-tool → depends on @opencoreagents/core — JSON-configured outbound HTTP tools ([`20-http-tool-adapter.md`](../reference/core/20-http-tool-adapter.md))
@opencoreagents/dynamic-definitions → depends on @opencoreagents/core + **adapters-http-tool** — definition store + **`hydrateAgentDefinitionsFromStore`** ([`21-dynamic-runtime-rest.md`](../reference/core/21-dynamic-runtime-rest.md))
...
|---------|------|----------------------|------------------|
| `@opencoreagents/core` | Engine loop, `Tool`/`Skill`/`Agent`/`Session` (optional **`expiresAtMs`** / **`SessionExpiredError`**), registries, built-in memory tools, vector tools, `system_send_message`, `InProcessMessageBus`, `RunStore`, **`buildEngineDeps`** / **`createRun`** / **`executeRun`**, **`dispatchEngineJob`** / **`EngineJobPayload`**, **`RunBuilder.onWait`**, **`effectiveToolAllowlist`**, optional **`toolTimeoutMs`**, **`dynamicDefinitionsStore`** (runtime `import()` of **`dynamic-definitions`**), adapters as interfaces | — | **Implemented** (Phases 1–4, 4b, 6–7; worker API + cluster docs — see `docs/roadmap/plan.md` **Progress snapshot**) |
| `@opencoreagents/utils` | Parsers, chunking, file-resolver ([`17-utils.md`](../reference/core/17-utils.md)) | — | **Implemented** (parsers: txt/md/json/csv/html; chunking: fixed_size/sentence/paragraph/recursive; file-resolver: local/http) |
| `@opencoreagents/adapters-http-tool` | `registerHttpToolsFromDefinitions`, `HttpToolConfig`, host allowlist + templates ([`20-http-tool-adapter.md`](../reference/core/20-http-tool-adapter.md)) | `core` | **Implemented** |
| `@opencoreagents/dynamic-definitions` | `DynamicDefinitionsStore` facade, `hydrateAgentDefinitionsFromStore`, upsert/sync ([`21-dynamic-runtime-rest.md`](../reference/core/21-dynamic-runtime-rest.md)) | `core`, `adapters-http-tool` | **Implemented** |
...
| `@opencoreagents/rag` | File-based RAG tools + skills ([`18-rag-pipeline.md`](../reference/core/18-rag-pipeline.md)) | `core`, `utils` | **Implemented** (system_file_read, system_file_ingest, system_file_list tools; rag, rag-reader skills) |
| `@opencoreagents/scaffold` | Programmatic `scaffold.initProject` / `generate*` ([`19-scaffold.md`](../reference/core/19-scaffold.md) API) | — *(no `workspace:*` in repo `package.json`)* | **Implemented** (TS templates, manifest) |
...
Subtrees under `packages/*/src` describe the **target** layout from `docs/reference/core/*`. Empty or minimal barrels are expected until the corresponding §12 phase lands.

```
agents/                    → repo root (Turborepo)
├── turbo.json
├── package.json
├── pnpm-workspace.yaml
├── tsconfig.base.json
...
│   │   └── vitest.config.ts
│   │
│   ├── adapters-upstash/             → @opencoreagents/adapters-upstash
│   │   ├── src/
...
│   │   └── tsup.config.ts
│   │
│   ├── adapters-openai/              → @opencoreagents/adapters-openai
│   │   ├── src/
│   │   │   ├── OpenAILLMAdapter.ts
...
│   │   └── tsup.config.ts
│   │
│   ├── adapters-bullmq/              → @opencoreagents/adapters-bullmq (BullMQ — priority workers)
│   │   ├── src/
│   │   │   ├── types.ts
│   │   │   ├── dispatch.ts           → re-export dispatchEngineJob from @opencoreagents/core
...
│   │
│   ├── rag/                          → @opencoreagents/rag (file-based RAG tools)
│   │   ├── src/
│   │   │   ├── tools/
...
These import from `@opencoreagents/utils` (parsers, chunking, file-resolver), so they live in the `rag` package.
...
3. Intersect with the **tool registry** and optional **`AgentRuntime.allowedToolIds`** → effective allowlist for **`ContextBuilder`** (today **`SecurityContext` does not further hide tools** in core — see [`11-scope-and-security.md`](../reference/core/11-scope-and-security.md) §2).
...
| `Agent.load(agentId, runtime, { session })` | Principal authorized on project; agent exists in namespace; **`AgentRuntime`** matches deployment ([`16-cluster-deployment.md`](../reference/core/16-cluster-deployment.md)). |
...
After Phase 1 (including rows 15–17): `pnpm turbo build --filter=@opencoreagents/core` and `pnpm turbo test --filter=@opencoreagents/core` must pass.
...
RunStore enables `wait`/`resume` across cluster nodes — see [`16-cluster-deployment.md §3`](../reference/core/16-cluster-deployment.md).

### Phase 5 — Job queue + cluster — `packages/adapters-bullmq` (**BullMQ priority**)

| # | Module | Notes | Status |
|---|--------|-------|--------|
| 24 | BullMQ queue + worker; `dispatchEngineJob` in **`core`** (`engine/dispatchJob.ts`), re-exported in **`adapters-bullmq`** | `packages/adapters-bullmq/src/` (`queue.ts`, `worker.ts`, `dispatch.ts`, `types.ts`); `packages/core/src/engine/dispatchJob.ts` | ✅ |
...
**BullMQ** is the primary production pattern — see [`16-cluster-deployment.md` §4](../reference/core/16-cluster-deployment.md). **QStash** remains an optional integration you build in the host app.
...
After Phase 6: `pnpm turbo build` builds all packages in topological order. `@opencoreagents/rag` depends on `core` + `utils`; `core` remains free of internal `workspace:*` dependencies.
...
**In-repo status:** Phases **0–4**, **4b** (RunStore), **5** (**`adapters-bullmq`** — BullMQ priority), and **6–8** (RAG, multi-agent, CLI + scaffold) are implemented — `pnpm turbo run build test lint` passes for all **fifteen** workspace packages under `packages/`. **CI:** `.github/workflows/ci.yml` runs the same pipeline on push/PR. **QStash** is not a package; integrate via HTTP if needed (see [`16-cluster-deployment.md`](../reference/core/16-cluster-deployment.md)). **Phase 9** (full integration hardening, including optional E2E with live keys) is partial. See **`docs/roadmap/plan.md` → Progress snapshot**.

---

## 13. Public API surface (`@opencoreagents/core`)
...
- **`new AgentRuntime({ runStore, … })`** — persist `waiting` runs; **`Agent.resume`** loads from the store (see `docs/reference/core/16-cluster-deployment.md`).
...
- **`@opencoreagents/adapters-bullmq`** — **`createEngineQueue`**, **`createEngineWorker`**; **`dispatchEngineJob(runtime, payload)`** and **`EngineJobPayload`** from **`@opencoreagents/core`**, re-exported (see `packages/adapters-bullmq/src/index.ts`).
...
| `core` | Security | **`projectId`** isolation in keys/adapters; prompt tool set = agent allowlist ∩ registry (+ optional **`AgentRuntime.allowedToolIds`**) — **`SecurityContext` is not applied in `ContextBuilder` yet** ([`11-scope-and-security.md`](../reference/core/11-scope-and-security.md) §2, [`technical-debt-security-production.md`](./technical-debt-security-production.md#1-security-integrity-and-production-readiness) §1); principal auth stays in the host **`SecurityLayer`** | Unit for allowlist/prefixes; host integration for HTTP |
...
17. **Tests colocated per package.** Each package has its own `tests/` directory and `vitest.config.ts`. Tests run in isolation via `pnpm turbo test --filter=@opencoreagents/PACKAGE`.
