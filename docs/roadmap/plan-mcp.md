...
| Area | Status |
|------|--------|
| **`@opencoreagents/mcp` or bundled MCP server** | **Not shipped** — no first-party MCP package in the workspace (see [`technical-debt-deferred.md`](./technical-debt-deferred.md#3-documentation) §3). |
| **Interop story** | **Documented at pattern level** in `15-consumers.md`. |
...
| **M4 — Multi-agent** | If hosts need “message agent B”, expose a single MCP tool that maps to **`system_send_message`** + bus semantics ([`14-communication-multiagent.md`](../reference/core/14-communication-multiagent.md)). | Document correlation / `wait` expectations. |
...
- Consumers summary: [`core/15-consumers.md`](../reference/core/15-consumers.md)
- REST plan (often under MCP): [`plan-rest.md`](./plan-rest.md)
- CLI (alternative surface): [`plan-cli.md`](./plan-cli.md)
- Brainstorm library + hooks philosophy: [`archive/brainstorm/06-library-adapters-cli.md`](../archive/brainstorm/06-library-adapters-cli.md)
