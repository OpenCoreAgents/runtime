...
> Roadmap for the **command-line** surface: what exists today vs the **library-parity** CLI sketched in [`archive/brainstorm/06-library-adapters-cli.md`](../archive/brainstorm/06-library-adapters-cli.md). Complements [`plan.md`](./plan.md) (monorepo/engine) and [`core/15-consumers.md`](../reference/core/15-consumers.md) §CLI.
...
| Phase | Goal | Gate |
|-------|------|------|
| **C1 — Design** | Choose scope for v1 (which commands; how **`AgentRuntime`** + env are loaded in a generated project). Document in [`scaffold.md`](./scaffold.md) or a short `docs/reference/core/` CLI subsection. | Written contract; no engine API changes required for C2. |
...
- Brainstorm: [`archive/brainstorm/06-library-adapters-cli.md`](../archive/brainstorm/06-library-adapters-cli.md) (folder layout, MVP bullets).
- Multi-agent + CLI snippet: [`archive/brainstorm/07-multi-agent-rest-sessions.md`](../archive/brainstorm/07-multi-agent-rest-sessions.md) §Extra CLI.
- Consumers overview: [`core/15-consumers.md`](../reference/core/15-consumers.md).
