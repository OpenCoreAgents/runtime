...
- **Run continuity:** the server keeps a Redis binding **`{definitions.keyPrefix}:chatBinding:{projectId}:{sessionId}`** → **`{ runId, agentId }`**. First message enqueues **`run`**; while the run is **`completed`**, further messages enqueue **`continue`** on that **`runId`**. If the run is **`failed`**, the next message starts a **new** **`runId`**. **`running`** → **409**; **`waiting`** → **409** (use plan REST **`POST /agents/:id/resume`** or a new chat session).
