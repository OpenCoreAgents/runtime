...
- **Model:** lazy-seed uses **`chat.defaultAgent.llm`** from the stack, then **`RUNTIME_CHAT_AGENT_MODEL`**, then **`RUNTIME_DEFAULT_LLM_MODEL`**, then conservative defaults — same precedence table as [configuration.md](./configuration.md#default-llm-model-environment). An existing **`chat`** row in Redis keeps its stored **`llm.model`** until you update or delete it.
